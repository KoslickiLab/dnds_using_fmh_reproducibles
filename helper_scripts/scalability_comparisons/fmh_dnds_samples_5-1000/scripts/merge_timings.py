...
def main():
    ...
    cont_pivot = cont_df.pivot(
        index=["sample", "wp_id"],
        columns="molecule",
        values="duration_sec"
        ).reset_index()

    cont_pivot = cont_pivot.rename(columns={
    ...
    merged = sig_clean.merge(
        cont_pivot,
        on=["sample", "wp_id"],
        how="left"
    )
    merged = merged.merge(
        fmh_df[["sample", "wp_id", "fmh_duration_sec"]],
        on=["sample", "wp_id"],
        how="left"
    )
    # --- Compute total time ---
    merged["total_time_sec"] = (
        ...
        + merged.get("fmh_duration_sec", 0).fillna(0)  # FMH time
    )

    merged["time_per_pairwise_sec"] = (
    merged["total_time_sec"] / 9
    )
